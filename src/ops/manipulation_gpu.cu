...
namespace ctranslate2 {
  namespace ops {
    ...
    static inline void scatter(const StorageView& input,
                               T* output_data,
                               const dim_t axis,
                               const dim_t offset,
                               const dim_t output_dim,
                               const dim_t inner_size,
                               const dim_t inner_bytes) {
      ...
      if (axis == 0) {
        primitives<D>::copy(input_data, output_data + offset * inner_size, input_size);
    ...
    void Concat::compute(const std::vector<const StorageView*>& inputs,
      ...
      for (const StorageView* input : inputs) {
        scatter<D, T>(*input, output_data, axis, offset, output_dim, inner_size, inner_bytes);
        offset += input->dim(axis);
      }
    }
    ...
    void Insert::compute(const StorageView& input, StorageView& output) const {
      ...
      const dim_t inner_bytes = inner_size * sizeof(T);
      T* output_data = output.data<T>();
      scatter<D, T>(input, output_data, axis, _index, output_dim, inner_size, inner_bytes);
    }

    template <Device D, typename T>
    static inline void gather(const T* input_data,
                              StorageView& output,
                              const dim_t axis,
                              const dim_t offset,
      ...
      if (axis == 0) {
        primitives<D>::copy(input_data + offset * inner_size, output_data, output_size);
    ...
    void Split::compute(const StorageView& input,
      ...
      for (StorageView* output : outputs) {
        gather<D, T>(input_data, *output, axis, offset, input_dim, inner_size, inner_bytes);
        offset += output->dim(axis);
      }
    }
    ...
    void Slide::compute(const StorageView& input, StorageView& output, const dim_t& index) const {
      ...
      const dim_t inner_bytes = inner_size * sizeof (T);
      const T* input_data = input.data<T>();
      gather<D, T>(input_data, output, axis, index, input_dim, inner_size, inner_bytes);
    }
...
#define DECLARE_IMPL(T)                                                 \
    ...
    Concat::compute<Device::CUDA, T>(const std::vector<const StorageView*>& inputs, \
                                     StorageView& output) const;        \
    template void                                                       \
    Insert::compute<Device::CUDA, T>(const StorageView& input,          \
                                     StorageView& output) const;        \
    template void                                                       \
