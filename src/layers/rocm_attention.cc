...
namespace ctranslate2 {
  namespace layers {
    ...
    RocmAttention::RocmAttention(const models::Model& model,
                                 ...
                                 bool is_decoder,
                                 Alibi* alibi)
      : AttentionLayer(model, scope, num_heads, self_attention, pre_norm, is_decoder, alibi, model.use_flash_attention())
      , _flash_attention(model.use_flash_attention())
    ...
    void RocmAttention::process_cross_attention(const StorageView& values,
      ...
      if (cached_keys == nullptr || cached_keys->empty()) {
        ...
        if (_merge_time_and_head_dims) { // MQA
          ...
          split_heads(fused_proj, 2 * _num_heads_kv, values_padder);
          const ops::Split split_op(_cache_time_dim == 1 ? 2 : 1);
    ...
    void RocmAttention::operator()(const StorageView& queries,
      ...
      if (!_self_attention) {
        ...
        if (_merge_time_and_head_dims) { // MQA
          ...
          const ops::Split split_op(2, {_num_heads * _d_head, _d_head, _d_head});
        ...
        if (_rotary_embeddings) {
          ...
          _rotary_embeddings->apply(queries_proj, offset, false);
          _rotary_embeddings->apply(keys_proj, offset, false);
    ...
    void RocmAttention::split_heads(StorageView& x,
                                    dim_t num_heads,
                                    const Padder* padder,
                                    dim_t beam_size) const {
      if (padder)
      ...
      if (_flash_attention) {
        x.reshape({batch_size, time, num_heads, _d_head});
      } else if (time == 1) {
        x.reshape({batch_size, num_heads, 1, _d_head});
      } else {
        x.reshape({batch_size, time, num_heads, _d_head});
    ...
    void RocmAttention::combine_heads(StorageView& x,
                                      dim_t num_heads,
                                      const Padder* padder,
                                      dim_t beam_size) const {
