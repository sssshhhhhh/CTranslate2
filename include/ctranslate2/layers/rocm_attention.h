...
namespace ctranslate2 {
  namespace layers {
    ...
    class RocmAttention : public AttentionLayer
    {
    public:
      RocmAttention(const models::Model& model,
      ...
      virtual bool has_positional_embeddings() const override {
        return _relative_position_keys || _relative_asymmetric_position_keys || _relative_attention_bias || _rotary_embeddings || _alibi;
      }

    protected:
      void process_cross_attention(const StorageView& values,
    ...
    private:
      void split_heads(StorageView& x,
                       dim_t num_heads,
                       const Padder* padder = nullptr,
                       dim_t beam_size = 1) const;

      void combine_heads(StorageView& x,
                         dim_t num_heads,
                         const Padder* padder = nullptr,
                         dim_t beam_size = 1) const;

      const bool _flash_attention;
      ...
      const bool _qk_norm_post_rope;
      const bool _keep_packed_qkv;
    };
  }
}
